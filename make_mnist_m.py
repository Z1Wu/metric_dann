...
class mnist_cls:
    def __init__(self, train:MNIST, test:MNIST):
        self.train_images = train.train_data.numpy()
        self.train_labels = train.train_labels.numpy()

        self.test_images = test.test_data.numpy()
        self.test_labels = test.test_labels.numpy()

mnist = mnist_cls(train_dataset, test_dataset)
...
train = create_mnistm(mnist.train_images)
print('Building test set...')
test = create_mnistm(mnist.test_images)

# Save dataset as pickle
mnistm_dir = os.path.abspath("./dataset/MNIST-M")
if not os.path.exists(mnistm_dir):
    os.mkdir(mnistm_dir)

training_set = (train, mnist.train_labels) # add label here
test_set = (test, mnist.test_labels)

with open(os.path.join(mnistm_dir,'training.pt'), 'wb') as f:
    torch.save(training_set, f)
with open(os.path.join(mnistm_dir,'test.pt'), 'wb') as f:
